...
namespace moleQule.Face.Skin06
{
	...
	/// </summary>
	public partial class SchemaMngSkinForm<T, C> : Skin04.SchemaMngSkinForm
		...
		where C : ReadOnlyBaseEx<C>
	{
		...
		protected override void SetFilter(bool on)
		{
			...
			try
			{
				SetMainList(on ? _filter_results : _sorted_list, SortProperty, SortDirection, on);
			}
			catch (Exception)
			{
				SetMainList(_sorted_list, SortProperty, SortDirection, on);
			}
